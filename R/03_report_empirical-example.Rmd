...
knitr::opts_chunk$set(fig.width=10, fig.height=7, echo = TRUE, message = FALSE, warning = FALSE)
```
...
The illustration uses openly accessible data from The Basic Education Evaluation System (Saeb) conducted by Brazil’s National Institute for Educational Studies and Research (Inep), available at <https://www.gov.br/inep/pt-br/acesso-a-informacao/dados-abertos/microdados/saeb>. It is also available as the `saeb` dataset in the `ivd` package.
...
```{r}
# We'll need to use `.summary_table()` and `.extract_to_mcmc()`, which are internal
# functions from `ivd`, that are not meant for direct use
...
posterior_samples <- do.call(rbind, lapply(ivd:::.extract_to_mcmc(out),
                                           FUN = function(x) x[, scale_ranef_pos]))
...
ggplot(u_ordered, aes(x = ordered, y = Median)) +
  geom_pointrange(
