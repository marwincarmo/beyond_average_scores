...
subtitle: "Study 1b: Main simulation"
author: Marwin Carmo
---
...
results_df <- results_raw |>
  ...
  tidyr::pivot_wider(
         names_from = metric,
         values_from = value) |>
  tidyr::replace_na(list(fn = 0)) |> #filter(n_students == 75, p_slab == 0) |> pull(sd_slab) |> unique()
  dplyr::mutate(
         tpr = tp / (tp + fn),
         fpr = fp / (fp + tn),
         tnr = tn / (tn + fp),
...
```{r}
results_df2 <- readRDS("../data/output/01_simulation_main_results_spiked.rds") |> 
  dplyr::mutate(ivd_sd_scl_Intc =  dplyr::coalesce(ivd_sd_scl_Intc, jags_sd_scl_Intc)) |> 
  dplyr::select(-jags_sd_scl_Intc) |> 
  dplyr::mutate(
  ...
  tidyr::pivot_longer(
    cols = matches("^(bias|rmse)_"),  # Metrics prefixed columns
    names_to = c("metric", "model"),
    names_pattern = "^(bias|rmse)_(.+)$",
    values_to = "value"
    ) |>
  # fix model names
  dplyr::mutate(model = dplyr::case_when(
